...
# took out Sex*Pclass
train.glm <- glm(Survived ~ Pclass + Family + Sex + Age + -1*Child +  Family + Mother, family = binomial, data = trainData)
...
kaggle4.sub <- cbind(PassengerId,survival)
colnames(kaggle4.sub) <- c("PassengerId", "Survived")
write.csv(kaggle4.sub, file = "kaggle4.csv", row.names = FALSE)
